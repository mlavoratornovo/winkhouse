...
public class ReportEngine {
	...
	public ArrayList<ReportMarkersVO> getextractReportMarkers(String reportPath){
		...
		try {
			...
			NodeList nl = new NodeList();//odfDocument.getContentDom().getElementsByTagName(MARKERELEMENT_TAGNAME);			
			for (int i = 0; i < nl.getLength(); i++) {
				ReportMarkersVO rmVO = new ReportMarkersVO();
				rmVO.setTipo(nl.item(i)/*nl.item(i).getAttributes().getNamedItem(MARKELEMENT_TAGTYPE).getNodeValue()*/);
				rmVO.setNome(nl.item(i)/*nl.item(i).getTextContent()*/);
				...
				while (itM.hasNext()){
					...
					for (int i = 0; i < nl.getLength(); i++) {
						...
						if (rmVO.getNome().equalsIgnoreCase(nl.item(i)/*nl.item(i).getTextContent()*/)){
							...
							odfPrintMarkerValues(new Node()/*nl.item(i)*/, 
