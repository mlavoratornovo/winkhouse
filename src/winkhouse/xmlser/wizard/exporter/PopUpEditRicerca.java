...
//		gdFillH.horizontalSpan = 2;
				...
				if (!ricerche.getNome().trim().equalsIgnoreCase("")){
					...
					if (!rh.saveRicerca(ricerche)){
						MessageDialog.openError(PlatformUI.getWorkbench()
												...
												"Si � verificato un errore nel salvataggio della ricerca");		
					}else{
	...
	private void bindRicerca(DataBindingContext bindingContext){
		...
		bindingContext.bindValue(WidgetProperties.text(SWT.Modify).observe(nome), 
								 PojoProperties.value("nome").observe(ricerche),
								 null, 
								 null);
		...
		bindingContext.bindValue(WidgetProperties.text(SWT.Modify).observe(descrizione), 
								 PojoProperties.value("descrizione").observe(ricerche),
								 null, 
								 null);
