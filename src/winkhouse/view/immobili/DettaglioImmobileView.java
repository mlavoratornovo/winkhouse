	...
	private void bindPosizioneImmobile(DataBindingContext bindingContext){
		if (immobile != null){
			bindingContext.bindValue(SWTObservables.observeText(
					triferimento,SWT.Modify), 
					PojoObservables.observeValue(immobile, "rif"),
					null, 
					null);
			
			bindingContext.bindValue(SWTObservables.observeText(
					tcitta,SWT.Modify), 
					PojoObservables.observeValue(immobile, "citta"),
					null, 
					null);
	
			bindingContext.bindValue(SWTObservables.observeText(
					tprovincia,SWT.Modify), 
					PojoObservables.observeValue(immobile, "provincia"),
					null, 
					null);
	
			bindingContext.bindValue(SWTObservables.observeText(
					tcap,SWT.Modify), 
					PojoObservables.observeValue(immobile, "cap"),
					null, 
					null);
	
			bindingContext.bindValue(SWTObservables.observeText(
					tzona,SWT.Modify), 
					PojoObservables.observeValue(immobile, "zona"),
					null, 
					null);
			
			bindingContext.bindValue(SWTObservables.observeText(
					tindirizzo,SWT.Modify), 
					PojoObservables.observeValue(immobile, "indirizzo"),
					null, 
					null);
			bindingContext.bindValue(SWTObservables.observeText(
					tncivico,SWT.Modify), 
					PojoObservables.observeValue(immobile, "ncivico"),
					null, 
					null);
		...
		tmq.addListener(SWT.KeyUp, new Listener(){
			...
			public void handleEvent(Event event) {
				...
				if (mq < 0){
					...
					decmq.setDescriptionText("La superficie in metri quadri non può essere minore di 0");
					f.getBody().redraw();
				}else{
					decmq.setImage(null);
		...
		tprezzo.addListener(SWT.KeyUp, new Listener(){
			...
			public void handleEvent(Event event) {
				...
				if (prezzo < 0){
					...
					decprezzo.setDescriptionText("Il prezzo non può essere minore di 0");
					f.getBody().redraw();
				}else{
		...
		tmutuo.addListener(SWT.KeyUp, new Listener(){
			...
			public void handleEvent(Event event) {
				...
				if (mutuo < 0){
					...
					decMutuo.setDescriptionText("Il mutuo non può essere minore di 0");
					f.getBody().redraw();
				}else{
		...
		ihVediCartellaAllegati.addMouseListener(new MouseListener(){
			...
			public void mouseUp(MouseEvent e) {
				...
				if ((getImmobile().getCodImmobile() != null) && (getImmobile().getCodImmobile() != 0)){
					...
					MessageDialog.openError(PlatformUI.getWorkbench().getActiveWorkbenchWindow().getShell(), 
											"Apertura cartella", 
											"Eseguire il salvataggio dell\'immobile per accedere a questa funzionalità");
				}
