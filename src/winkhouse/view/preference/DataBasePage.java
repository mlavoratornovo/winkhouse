	...
	protected void performApply() {
		...
		if (oldbundlevalue != bundleDB.getSelection()){
			if (bundleDB.getSelection()){
				if (MessageDialog.openConfirm(getShell(), "Riavvio winkhouse", "La voce \"Utilizza base dati autonoma\" � stata selezionata.\n"
						 												 + "Se nel pc � attivo il programma winkhouseDBAgent, chiuderlo tramite File -> Exit."
						 												 + "Per rendere effettivo il cambiamento della base dati \n"
						 												 + "� necessario riavviare il programma, RIAVVIARE ORA ?")){
					HSQLDBHelper.getInstance().chiudiDatabase();
					PlatformUI.getWorkbench().restart();	 													 
						 												 }
				
			}else{
				if (MessageDialog.openConfirm(getShell(), "Riavvio winkhouse", "La voce \"Utilizza base dati autonoma\" � stata cambiata, \n"
																	 + " per rendere effettivo il cambiamento della base dati \n"
																	 + "� necessario riavviare il programma, RIAVVIARE ORA ?")){
					HSQLDBHelper.getInstance().chiudiDatabase();
					PlatformUI.getWorkbench().restart();
