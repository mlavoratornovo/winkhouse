				...
				if (tmpfine != null && tmp != null){ 
					if (tmp.compareTo(tmpfine) <= 0){
						...
						MessageDialog.openWarning(Activator.getDefault()
								   						   ...
								   						   .getShell(),
								   				  "Attenzione !", 
												  "La data/ora appuntamento è maggiore della data/ora di fine. \n al salvataggio verrà valorizzata con il valore della data/ora fine");
					}
				}else{
					appuntamento.setDataAppuntamento(tmp);
				}
				...
				try {
					...
					if (tmpfine.compareTo(tmpinizio) >= 0){
						...
						MessageDialog.openWarning(Activator.getDefault()
								   						   ...
								   						   .getShell(),
								   				  "Attenzione !", 
												  "La data/ora fine appuntamento è minore della data di inizio. \n al salvataggio verrà valorizzata con il valore della data/ora inizio");
...
/*            	getColloquio().setDataColloquio(
	...
	private void bindAppuntamento(DataBindingContext bindingContext){
		...
		bindingContext.bindValue(SWTObservables.observeText(
								 ...
								 PojoObservables.observeValue(appuntamento, "luogo"),
								 null, 
								 null);
		...
		bindingContext.bindValue(SWTObservables.observeText(descrizione,SWT.Modify), 
								 PojoObservables.observeValue(appuntamento, "descrizione"),
								 null, 
								 null);
