...
public class CriteriRicercaColloquio extends WizardPage {
	...
	public void createControl(Composite parent) {
		...
		ihNew.addMouseListener(new MouseListener(){
			...
			public void mouseUp(MouseEvent e) {
				if ((criteri.size() == 0) ||
					(((Colloquicriteriricerca)criteri.get(criteri.size() - 1)).getGettermethodname() != null)){
		...
		ihCancella.addMouseListener(new MouseListener(){
			...
			public void mouseUp(MouseEvent e) {
				...
				if ((ss != null) && (ss.getFirstElement() != null)){
					criteri.remove((Colloquicriteriricerca)ss.getFirstElement());
					reloadLineNumber();
					Collections.sort(criteri, comparatorLineNumber);					
					tvCriteri.refresh();
				}
		...
		tvCriteri = ctuf.getSearchImmobiliCriteriaTable(container,
														new ArrayList(((ColloquiWizard)getWizard()).getColloquio()
				   																	 .getColloquicriteriricercas()));
	...
	public void loadCriteriColloquio(){
		criteri = new ArrayList<Colloquicriteriricerca>();
		//TODO cayenne ((ColloquiWizard)getWizard()).getColloquio().setCriteriRicerca(criteri);
	}
	
	private void reloadLineNumber(){
		if (criteri != null){
			int count = 0;
			for (Iterator<Colloquicriteriricerca> iterator = criteri.iterator(); iterator.hasNext();) {
