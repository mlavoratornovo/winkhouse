					...
					if (o != null){
						...
						if (!caDAO.saveUpdate(affittiAnagrafica, con, false)){					
							MessageDialog.openError(Activator.getDefault().getWorkbench().getActiveWorkbenchWindow().getShell(),
													...
													"Si � verificato un errore nel salvataggio dell'associazione affitto anagrafica : " + 
				...
				while (ite.hasNext()){
					...
					if (!caDAO.deleteAffittiAnagraficheByID(affittoAnagrafiche.getCodAffittiAnagrafiche(), con, false)){
						MessageDialog.openError(Activator.getDefault().getWorkbench().getActiveWorkbenchWindow().getShell(),
												...
												"Si � verificato un errore nella cancellazione dell'associazione affitto anagrafica : " + 
				...
				if (o != null){
					...
					if (!arDAO.saveUpdate(affittiRata, con, false)){					
						MessageDialog.openError(Activator.getDefault().getWorkbench().getActiveWorkbenchWindow().getShell(),
								...
								"Si � verificato un errore nel salvataggio dell'associazione affitto rata : " + 
			...
			while (ite.hasNext()){
				...
				if (!arDAO.deleteAffittiRateByID(affittoRata.getCodAffittiRate(), con, false)){
					MessageDialog.openError(Activator.getDefault().getWorkbench().getActiveWorkbenchWindow().getShell(),
											...
											"Si � verificato un errore nella cancellazione dell'associazione affitto rata : " + 
			...
			while (it.hasNext()){
				...
				if (o != null){
					...
					if (!asDAO.saveUpdate(affittiSpesa, con, false)){					
						MessageDialog.openError(Activator.getDefault().getWorkbench().getActiveWorkbenchWindow().getShell(),
								"Errore salvataggio colloqui rata", 
								"Si � verificato un errore nel salvataggio dell'associazione affitto spesa : " + 
			...
			while (ite.hasNext()){
				...
				if (!asDAO.deleteAffittiSpeseByID(affittoSpesa.getCodAffittiSpese(), con, false)){
					MessageDialog.openError(Activator.getDefault().getWorkbench().getActiveWorkbenchWindow().getShell(),
											...
											"Si � verificato un errore nella cancellazione dell'associazione affitto spesa : " + 
	...
	public HashMap saveAffitto(AffittiModel affitto, Connection con){
		...
		if (checkPeriodoAffitto(affitto)){
			...
			MessageDialog.openError(PlatformUI.getWorkbench()
									...
									"L'immobile risulta gi� affittato nel periodo selezionato");
			returnValue.put(RESULT_AFFITTI_DB, false);
		}
		return returnValue;
