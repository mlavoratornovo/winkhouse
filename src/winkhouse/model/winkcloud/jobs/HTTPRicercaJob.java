...
public class HTTPRicercaJob extends Job {
	...
	protected IStatus run(IProgressMonitor monitor) {
	    ...
	    if (!httpHelper.queryFileExist(ricerca.getMonitor().getConnector(), 
	    	 						   filename, 
	    							   ricerca.getMonitor().getConnector().getCode())){
	    	
	    	if (httpHelper.uploadQueryRequest(ricerca.getMonitor().getConnector(), 
					  						  requestZipFile, 
					  						  ricerca.getMonitor().getConnector().getCode())){
	    		this.setChkvar(true);	
			}else{
				this.setChkvar(false);
			}
	    ...
	    String cloudid = WinkhouseUtils.getInstance().getPreferenceStore()
    	...
    	while (this.isChkvar()){
    		...
    		ResponseByCodeQueryName[] queryResponseData = httpHelper.getQueryResposesByCodeQueryName(ricerca.getMonitor().getConnector(),
    												   												 ricerca.getMonitor().getConnector().getCode(), 
    												   												 filename);
    		if (queryResponseData != null){
	    		...
	    		for (int i = 0; i < queryResponseData.length; i++) {
	    			...
	    			if ((fileFinds.get(queryResponseData[i].filename) == null) && (!queryResponseData[i].winkcloudid.equalsIgnoreCase(cloudid))){
	    				
		        		String downfilename = queryResponseData[i].filename;
		        		String pathdownfile = pathWorkingFolder + File.separator + downfilename;
		        		
		        		File fdownfile = new File (pathdownfile);
			
		    			if (httpHelper.downloadResponse(ricerca.getMonitor().getConnector(),
		    											ricerca.getMonitor().getConnector().getCode(),
		    											queryResponseData[i].idfile,
		    											fdownfile)){
		
		        			String unzipFolder = pathWorkingFolder + File.separator + downfilename.replace(".zip", "");
		        			File fUnzipFolder = new File(unzipFolder);
		        			
		        			if (!fUnzipFolder.exists()){
		        				fUnzipFolder.mkdirs();
		        			}
		        			
		        			ZipUtils zu = new ZipUtils();
		        			if (zu.unZip4jArchivio(pathdownfile, unzipFolder)){
		        						        				
		        				ImporterVO iVO = new ImporterVO();
		        				iVO.setWinkCloudPathFile(pathdownfile);
		        				iVO.setPathExportFile(unzipFolder);
		        				iVO.setWinkCloudPathFile_receiveDate(new Date());
		        				
		        				ImporterHelper ih = new ImporterHelper();
		        				ih.importDataNoShell(iVO);
		        						        				
		        				ricerca.getImporterVO().add(iVO);		        				
		        				fileFinds.put(downfilename, fdownfile);
		        				ricerca.getTvMonitors().getControl().getDisplay().asyncExec(new Runnable() {
									
									@Override
									public void run() {
										ricerca.getTvMonitors().refresh();
										ricerca.getTvMonitors().expandToLevel(4);
				...
				httpHelper.deleteQueryRequest(ricerca.getMonitor().getConnector(),
				   		  					  ricerca.getMonitor().getConnector().getCode(), 
				   		  					  filename);
